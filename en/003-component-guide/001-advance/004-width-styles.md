...
id: width-styles
title: Width
description: Set the width of the layer content area
...
# Width Style Component

_Width refers to the width of the layer content area, used to control the horizontal size of the layer content area._
...
___Most of the time you don't need to use the width component, because most of the content should be adjusted to fit the size of the content responsively.___

## Number Adjustment

Number adjustment can set the width of the layer content area, the number part can be input directly or dragged with the slider, only positive numbers are supported, the supported units include:
...
In some cases, you may want to limit the size of the layer width, you can check the `Max Limit` or `Min Limit` option, and then set the maximum or minimum value, the units are the same as the number adjustment part.

## Percentage Adjustment

When your layer positioning is modified to `Absolute`, you can set the percentage of the layer width relative to its `Datum`, the width will be calculated based on the percentage of the `Datum` and the value you set, the value input in this mode supports negative numbers.
